...
- [Core Architecture & Intelligence Subsystems](#core-architecture--intelligence-subsystems)
...
## Core Architecture & Intelligence Subsystems

FORENZA's backend architecture is structured around 10 core intelligence subsystems, providing rigorous statistical computation, data governance, and high-concurrency execution.
...
└─────────────────────────────────────────────────────────────────────────────────────────┘
        │
        ├─► Autosomal STR & Kinship Engine (CODIS 24, Likelihood Ratio LR)
        ├─► Probabilistic Genotyping Engine (Metropolis-Hastings MCMC)
        ├─► Forensic Phenotyping & Ancestry (HIrisPlex-S 41-SNP Model)
        ├─► Statistical Population Genetics (NRC II Bounds & Fst Distance)
        ├─► ENFSI Legal Report Generator & Compliance Auditor
        ├─► High-Throughput Batch Processing Engine (Worker Semaphore)
        ├─► Validation Lab & Synthetic Profile Generator (ROC / FIR Curves)
        ├─► Distributed Federated Network (P2P Discovery & Node Signatures)
        ├─► Cryptographic Ledger & ZKP Privacy Auditor (Circom ZK-SNARKs)
        └─► System Integrity Engine (HMAC Hash Chaining & Health Metrics)
```
...
FORENZA integrates 5 specialized domain engines into its central workflow:
...
└─────────────────────────────────────────────────────────────────────────────────────────┘
        │
        ├─► Lineage DNA (Y-FILER 23 Y-STR, Argus X-12 X-STR, mtDNA rCRS)
        ├─► Missing Persons & DVI (Interpol AM/PM Cross-Reconciliation)
        ├─► Human Identification (Multi-Modal Joint LR Synthesis & LCN Audit)
        ├─► Forensic Anthropology (Sex, Suchey-Brooks Age, Trotter-Gleser Stature)
        └─► Forensic Entomology (Accumulated Degree Hours ADH & PMI_min Estimator)
```
