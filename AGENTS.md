...
### 1. Source of Truth
Every formula, equation, constant, algorithm, threshold, benchmark vector, and biophysical model across all 35 biocomputational modules **MUST derive exclusively and verbatim from the 7 research specifications** in the `research/` directory:
...
- [`pillar_7_geo_forensic_intelligence_research.md`](file:///c:/Users/Yusuf/str-analysis/research/pillar_7_geo_forensic_intelligence_research.md): Multi-Isotope Bivariate Normal Isoscape ($\delta^{18}\text{O}, \delta^2\text{H}, ^{87}\text{Sr}/^{86}\text{Sr}$), Soil Chemistry Mahalanobis Mineralogy (XRF/XRD), Palynological Forensic Pollen Assemblages & Bray-Curtis Dissimilarity, Rossmo CGT Geographic Profiling ($\phi=1.0, B=1.6\text{ km}, k=1.0$), Multi-Criteria Bayesian Evidence Fusion & 2D Adaptive KDE.
