#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# FORENZA — Global Multi-Node Deployment Script
...
generate_mtls_certs() {
    ...
    if [ ! -f "${CERTS_DIR}/ca.key" ]; then
        ...
        openssl req -new -x509 -days 365 -key "${CERTS_DIR}/ca.key" \
            ...
            -subj "/CN=FORENZA Root CA/O=Forensic Network/C=INT" 2>/dev/null
    ...
    if [ ! -f "${CERTS_DIR}/orchestrator.key" ]; then
        ...
        openssl req -new -key "${CERTS_DIR}/orchestrator.key" \
            ...
            -subj "/CN=orchestrator/O=FORENZA/C=INT" 2>/dev/null
    ...
    for node_id in "${!NODES[@]}"; do
        if [ ! -f "${CERTS_DIR}/${node_id}.key" ]; then
            ...
            openssl req -new -key "${CERTS_DIR}/${node_id}.key" \
                ...
                -subj "/CN=${node_id}/O=FORENZA/C=${code}" 2>/dev/null
...
services:
  ...
  orchestrator:
    ...
    ports:
      - "8000:8000"
    environment:
      - FORENZA_ROLE=orchestrator
      - FORENZA_MTLS_CA=/certs/ca.crt
      - FORENZA_MTLS_CERT=/certs/orchestrator.crt
      - FORENZA_MTLS_KEY=/certs/orchestrator.key
    volumes:
...
HEADER
  ...
  ${node_id}:
    build:
      context: ../backend
      dockerfile: Dockerfile
    container_name: forenza-${node_id}
    ports:
      - "${port}:8000"
    environment:
      - FORENZA_ROLE=node
      - FORENZA_NODE_ID=${node_id}
      - FORENZA_COUNTRY=${code}
      - FORENZA_CITY=${city}
      - FORENZA_ORCHESTRATOR_URL=http://172.28.0.2:8000
      - FORENZA_SYNTHETIC_PROFILES=${profiles}
      - FORENZA_MTLS_CA=/certs/ca.crt
      - FORENZA_MTLS_CERT=/certs/${node_id}.crt
      - FORENZA_MTLS_KEY=/certs/${node_id}.key
      - FORENZA_ZKP_ENABLED=true
      - FORENZA_DSPY_ENABLED=true
    volumes:
      - ../infra/certs:/certs:ro
    depends_on:
      orchestrator:
...
cmd_up() {
    echo ""
    echo -e "${BOLD}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}  FORENZA — Global Network Deployment${NC}"
    echo -e "${BOLD}═══════════════════════════════════════════════════════════${NC}"
    echo ""
