...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lower.includes("str") || lower.includes("lokus") || lower.includes("loci") || lower.includes("lr") || lower.includes("match") || lower.includes("codis")) {
      replyText = isTr
        ? "CODIS 24 lokus analizi sonuçlandı: Numune ile şüpheli profili arasında 24 lokusta (D3S1358, vWA, FGA, D8S1179, D21S11, D18S51 vb.) tam alel uyumu tespit edilmiştir. İki katkıcılı MCMC dekonvolüsyonu ile Birleşik Olabilirlik Oranı (Combined Likelihood Ratio) LR = 1.84 × 10¹⁸ olarak hesaplanmıştır. Bu sonuç SWGDAM standartlarında 'Kesin İdentifikasyon Desteği' kategorisindedir."
        : "CODIS 24 loci analysis complete: Suspect profile shows complete allele concordance across 24 loci (D3S1358, vWA, FGA, D8S1179, D21S11, D18S51, etc.). 2-contributor MCMC deconvolution yields Combined Likelihood Ratio LR = 1.84 × 10¹⁸, establishing 'Conclusive Support for Identity' under SWGDAM / ENFSI guidelines.";
      ...
      replyText = isTr
        ? "HIrisPlex-S (24-SNP) DNA Fenotipleme Çıkarımı:\n• Göz Rengi: %94.2 Mavi (HERC2 rs12913832 AA homozigot)\n• Ten Rengi Fototipi: %88.7 Fitzpatrick Tip I/II Açık Ten (SLC24A5/SLC45A2)\n• Saç Morfolojisi: %91.4 Düz, Kumral/Kahverengi tonu."
        : "HIrisPlex-S (24-SNP) DNA Phenotyping Inference:\n• Eye Color: 94.2% Blue (HERC2 rs12913832 AA homozygous)\n• Skin Phototype: 88.7% Fitzpatrick Type I/II Fair (SLC24A5/SLC45A2)\n• Hair Morphology: 91.4% Straight, Light Brown hue.";
