...
Write-Host "  Forensic Evidence Operating System — Air-Gapped Workstation Engine" -ForegroundColor Cyan
Write-Host "  ISO/IEC 17025:2017 Aligned • Zero External Cloud Telemetry • 35 Subsystems"
Write-Host "  ──────────────────────────────────────────────────────────────────────────`n"
...
if (-not (Test-Path $certFile) -or -not (Test-Path $keyFile)) {
    ...
    if (Get-Command openssl -ErrorAction SilentlyContinue) {
        ...
        # Note: In production or without openssl, openssl in container or cert-manager handles keys
    }
...
if ($healthy) {
    ...
    Write-Host "  📊 REST API Documentation: https://localhost:8443/docs" -ForegroundColor Cyan
    Write-Host "  🛡️ Security Status:        Air-Gapped (Zero Cloud Outbound)" -ForegroundColor Green
